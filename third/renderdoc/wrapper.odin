...
load_api :: proc(
	...
	when LOAD_RENDERDOC && ODIN_OS == .Windows {
		...
		if !os.exists(INSTALL_ROOT) {
			log.errorf("renderdoc install root %v doesnt exist", INSTALL_ROOT)
		...
		if !found {
			log.errorf(
				"could not find symbol %v in %v, %v",
				GET_API_SYMBOL,
				dll_path,
				dynlib.last_error(),
			)
		...
		GetAPI := cast(GetAPIProc)symbol_ptr
